...
// https://stackoverflow.com/a/25060605/8161267
function splitNoParenSemicolon(s) {
    var left= 0, right= 0, A= [], M= s.match(/([^()]+)|([()])/g), L= M.length, next, str= '';
    for(var i= 0; i<L; i++){
        next= M[i];
        if(next=== '(')++left;
...
module.exports.splitNoParenComma = splitNoParenComma;
module.exports.splitNoParenSemicolon = splitNoParenSemicolon;
